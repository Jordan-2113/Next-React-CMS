...
const handler = async (req, res) => {
    ...
    try {
        ...
        const [ rowsUpdated ] = await doctors.update(doctor, { where: { id: value.id } }, { transaction });
        ...
        await doctor_attainments.destroy({ where: { doctorId: value.id } }, { transaction });
        await doctor_specialty.destroy({ where: { doctorId: value.id } }, { transaction });
        ...
        res.status(500).send({ message: "Error while updating model" });
        return;
    }
};
